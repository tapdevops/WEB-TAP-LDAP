...
class ExportController extends Controller
{
	...
	// http://app.tap-agri.com/mobileinspectiondev/ins-msa-dev-auth
	// http://app.tap-agri.com/mobileinspectiondev/ins-msa-dev-auth/api/v1.0/abc/abc
	...
	public function sync_tapdw_tr_inspection(Request $req)
	{
	 ...
	 * --------------------------------------------------------------------------
	 */
	public function sync_mobile_estate_tr_ebcc_kualitas(Request $req)
	{
		$client = new \GuzzleHttp\Client();

		// $start_date = date('Ym1', strtotime('-5 day'));
		// $end_date = date('Ymd', strtotime('-1 day'));
			...
			foreach ($result['data'] as $inspeksi) {
				...
				if ($check[0]->count == 0) {
					foreach ($inspeksi['DETAIL'] as $detail) {
						if ($detail['CONTENT_INSPECTION_CODE'] == 'CC0002') {
							$ins_CC0002_pokok_panen = intval($detail['VALUE']);
						}
						if ($detail['CONTENT_INSPECTION_CODE'] == 'CC0004') {
							$ins_CC0004_piringan = intval($detail['VALUE']);
						}
						if ($detail['CONTENT_INSPECTION_CODE'] == 'CC0005') {
							$ins_CC0005_tph = intval($detail['VALUE']);
						}
					}
